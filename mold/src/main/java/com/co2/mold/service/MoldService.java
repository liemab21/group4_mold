...
import com.co2.mold.model.mold.MoldForecast;
...
public class MoldService {
    ...
    public MoldForecast calculateForecast() {
        List<Mold> recentData = moldRepository.findTop10ByOrderByDatetimeDesc();

        if (recentData.isEmpty()) {
            throw new IllegalStateException("No mold data found");
        }

        Map<MoldRisk, Long> riskFrequency = recentData.stream()
                .collect(Collectors.groupingBy(Mold::getMoldRisk, Collectors.counting()));

        MoldRisk mostCommonRisk = riskFrequency.entrySet().stream()
                .max(Map.Entry.comparingByValue())
                .map(Map.Entry::getKey)
                .orElse(MoldRisk.VERY_LOW);

        return new MoldForecast(LocalDate.now().plusDays(1), mostCommonRisk);
    }
